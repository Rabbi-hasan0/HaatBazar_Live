...
# ❌ Direct Import এড়াতে ShopOwnerProfile ইমপোর্ট বাদ দেওয়া হয়েছে।
# ForeignKey-তে সরাসরি 'accounts.ShopOwnerProfile' ব্যবহার করা হয়েছে।
...
class ProductMainCategory(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.cat_slug and self.main_cat_name:
            ...
            num = 1
            while (
                ProductMainCategory.objects.filter(
                    shop=self.shop, cat_slug=slug
                )
                .exclude(pk=self.pk)
                .exists()
            ):
...
class ProductSubCategory(models.Model):
    ...
    created_by = models.ForeignKey(
        "accounts.ShopOwnerProfile",
        on_delete=models.CASCADE,
        related_name="sub_category_created_by",
    )
    updated_by = models.ForeignKey(
        "accounts.ShopOwnerProfile",
        on_delete=models.CASCADE,
        ...
        if not self.sub_cat_slug and self.sub_cat_name:
            ...
            while (
                ProductSubCategory.objects.filter(
                    shop=self.shop, sub_cat_slug=slug
                )
                .exclude(pk=self.pk)
                .exists()
            ):
...
class Product(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.product_slug and self.product_name:
            ...
            num = 1
            while (
                Product.objects.filter(shop=self.shop, product_slug=slug)
                .exclude(pk=self.pk)
                .exists()
            ):
                slug = f"{base_slug}-{num}"
    ...
    def average_rating(self):
        return (
            self.reviews.filter(is_active=True).aggregate(models.Avg("rating"))[
                "rating__avg"
            ]
            or 0
        )
...
class InventoryActivityLog(models.Model):
    ...
    module_type = models.CharField(
        max_length=15, choices=MODULE_CHOICES, db_index=True
    )
    item_name = models.CharField(max_length=255)
    action_type = models.CharField(
        max_length=10, choices=ACTION_CHOICES, db_index=True
    )
...
class Wishlist(models.Model):
    ...
    shop = models.ForeignKey(
        "shops.Shop", on_delete=models.CASCADE, db_index=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class ProductReview(models.Model):
    ...
    created_by = models.ForeignKey(
        "accounts.ShopOwnerProfile",
        on_delete=models.CASCADE,
        related_name="product_review_created_by",
        db_index=True,
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class Event(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            num = 1
            while (
                Event.objects.filter(shop=self.shop, slug=slug)
                .exclude(pk=self.pk)
                .exists()
            ):
                slug = f"{base_slug}-{num}"
