...
class Shop(models.Model):
    ...
    shop_slug = models.SlugField(
        max_length=120, db_index=True, unique=True, null=True, blank=True
    )
    shop_logo = models.ImageField(
        upload_to='shop_logos/', null=True, blank=True
    )
    ...
    banner_image = models.ImageField(
        upload_to='shops/banners/', null=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        if not self.shop_slug:
            self.shop_slug = generate_unique_slug(
                self.shop_name, Shop, 'shop_slug'
            )
...
class ShopActivityLog(models.Model):
    ...
    def __str__(self):
        return (
            f'{self.shop.shop_name} - {self.user.username} - {self.action}'
        )
...
class Coupon(models.Model):
    ...
    user = models.ManyToManyField(
        settings.AUTH_USER_MODEL, blank=True, related_name='user_coupons'
    )
...
class CouponActivityLog(models.Model):
    ...
    shop = models.ForeignKey(
        'Shop',
        on_delete=models.CASCADE,
        related_name='coupon_activities',
        db_index=True,
        null=True,
        blank=True,
    )
...
class Events(models.Model):
    ...
    banner_mobile = models.ImageField(
        upload_to='events/mobile/', null=True, blank=True
    )

    # ✅ পরিবর্তন: Product ক্লাসের জায়গায় 'products.Product' স্ট্রিং ব্যবহার করা হয়েছে
    product = models.ManyToManyField(
        'products.Product', related_name='shop_app_products_events', blank=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['shop', 'slug'], name='unique_shop_event_slug'
            )
        ]

    def __str__(self):
        return f'{self.title} ({self.shop.shop_name if self.shop else ""})'
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            num = 1
            while (
                Events.objects.filter(shop=self.shop, slug=slug)
                .exclude(pk=self.pk)
                .exists()
            ):
    ...
    def is_running(self):
        now = timezone.now()
        return (
            self.status == 'active' and self.start_time <= now <= self.end_time
        )
...
class EventActivity(models.Model):
    ...
    event = models.ForeignKey(
        Events, on_delete=models.SET_NULL, null=True, related_name='activities'
    )
    event_title_backup = models.CharField(
        max_length=255,
        help_text='Event ডিলিট হয়ে গেলেও যেন নাম ট্র্যাক করা যায়',
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
    ...
    changes_logged = models.TextField(
        blank=True, null=True, help_text='কী কী চেঞ্জ হয়েছে তার ডিটেইলস'
    )
    ip_address = models.GenericIPAddressField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class ShopReview(models.Model):
    ...
    is_visible = models.BooleanField(
        default=True, db_index=True, verbose_name='Is Visible on Site'
    )
    created_at = models.DateTimeField(auto_now_add=True, db_index=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f'{self.customer.user.username} -> {self.shop.shop_name} ({self.rating}★)'
